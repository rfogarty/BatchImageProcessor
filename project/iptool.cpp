...
int main (int argc, char** argv)
{
   ...
   typedef Image<PixelT> ImageT;
	...
	while(fgets(str,MAXLEN,fp) != NULL) {
		pch = strtok(str, " ");
      ImageT src = readPGMFile<PixelT>(std::string(pch));
	   ImageT tgt(src.rows(),src.cols());
      ...
      else if (strncasecmp(pch,"binarizeDT",MAXLEN)==0) {
         /* Thresholding */
         pch = strtok(NULL, " ");
         int tlow = atoi(pch);
         pch = strtok(NULL, " ");
         int thigh = atoi(pch);
         utility::binarizeDouble(src,tgt,tlow,thigh);
      }
      ...
      writePGMFile(std::string(outfile),tgt);
	}
	fclose(fp);
	return 0;
}
